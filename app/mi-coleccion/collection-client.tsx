...
export default function CollectionClient({ initialStickers, userId, collectionName, collectionSlug, collections }: { initialStickers: Sticker[]; userId: string; collectionName: string; collectionSlug: string; collections: CollectionOption[] }) {
  ...
  const text = language === "es" ? {
    ...
    available:"cromos disponibles", all:"Todos", search:"Número, cromo o equipo", marked:"cromos marcados", automatic:"Guardado automático activado", remove:"Desmarcar"
  } : {
    ...
    available:"stickers available", all:"All", search:"Number, sticker or team", marked:"marked stickers", automatic:"Automatic saving enabled", remove:"Unmark"
  };
  ...
  return <div className="min-h-screen pb-24 md:pb-12">
    ...
    <main className="mx-auto max-w-7xl px-4 py-8 md:px-8 md:py-12">
      ...
      <section className="mb-6 overflow-hidden rounded-2xl border border-[#173d2a]/15 bg-white">
        ...
        <div className="grid grid-cols-2 border-t border-[#173d2a]/10 bg-[#fbfaf5] text-center"><Stat value={counts("wanted")} label={text.missing}/><Stat value={counts("duplicate")} label={text.duplicates}/></div>
      </section>

      <div className="mb-5 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex gap-2 overflow-x-auto pb-1">{([['all',text.all],['wanted',text.missing],['duplicate',text.duplicates]] as const).map(([key,label]) => <button key={key} onClick={() => setFilter(key)} className={`whitespace-nowrap rounded-full px-4 py-2 text-sm font-bold ${filter===key?'bg-[#17231b] text-white':'border border-[#173d2a]/15 bg-white'}`}>{label}</button>)}</div>
        <div className="flex gap-2"><label className="flex min-w-0 flex-1 items-center gap-2 rounded-xl border border-[#173d2a]/15 bg-white px-3 lg:w-72"><Search size={18} className="text-[#718078]"/><input value={query} onChange={e=>setQuery(e.target.value)} aria-label={text.search} placeholder={text.search} className="w-full bg-transparent py-3 outline-none"/></label><button aria-label="Vista de cuadrícula" className="rounded-xl bg-[#17231b] p-3 text-white"><Grid2X2 size={19}/></button><button aria-label="Vista de lista" className="rounded-xl border border-[#173d2a]/15 bg-white p-3"><List size={19}/></button></div>
      </div>
