...
import { Album, Eye, Grid2X2, Layers3, List, Plus, Search, X } from "lucide-react";
...
export default function CollectionClient({ initialStickers, userId, collectionName, collectionSlug, collections }: { initialStickers: Sticker[]; userId: string; collectionName: string; collectionSlug: string; collections: CollectionOption[] }) {
  ...
  const teamProgress = useMemo(() => {
    const summary = new Map<string, { total: number; marked: number }>();
    stickers.forEach(sticker => { const current = summary.get(sticker.section) ?? { total: 0, marked: 0 }; current.total += 1; if (sticker.status) current.marked += 1; summary.set(sticker.section, current); });
    return [...summary.entries()].map(([name, values]) => ({ name, ...values })).filter(item => item.marked > 0).sort((a, b) => b.marked - a.marked || a.name.localeCompare(b.name, "es")).slice(0, 4);
  }, [stickers]);
  const categoryProgress = useMemo(() => {
    const summary = new Map<string, number>();
    stickers.filter(sticker => sticker.status).forEach(sticker => { const category = sticker.category || "Colección base"; summary.set(category, (summary.get(category) ?? 0) + 1); });
  ...
  return <div className="min-h-screen pb-24 md:pb-12">
    ...
    <main className="mx-auto max-w-7xl px-4 py-8 md:px-8 md:py-12">
      ...
      {(teamProgress.length>0||categoryProgress.length>0)&&<section className="mb-6 grid gap-4 lg:grid-cols-[1.35fr_.65fr]"><div className="rounded-2xl bg-[#17231b] p-5 text-white"><div className="flex items-center gap-2"><Layers3 size={18} className="text-[#c9f31d]"/><h2 className="font-black">Tu colección por equipos</h2></div><div className="mt-5 grid gap-4 sm:grid-cols-2">{teamProgress.map(team=><div key={team.name}><div className="mb-2 flex items-center justify-between gap-3 text-xs"><span className="truncate font-bold">{team.name}</span><span className="shrink-0 text-white/55">{team.marked} de {team.total}</span></div><div className="h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-[#c9f31d]" style={{width:`${Math.max(5,(team.marked/team.total)*100)}%`}}/></div></div>)}</div></div><div className="rounded-2xl border border-[#173d2a]/15 bg-white p-5"><h2 className="font-black">Categorías marcadas</h2><div className="mt-4 flex flex-wrap gap-2">{categoryProgress.map(([category,count])=><span key={category} className="rounded-full bg-[#e5efd9] px-3 py-2 text-xs font-bold text-[#164f35]">{category} · {count}</span>)}</div></div></section>}
...
const palettes=[{dark:"#123e2a",accent:"#c9f31d",soft:"#edf7d6"},{dark:"#17231b",accent:"#ff6a3d",soft:"#fff0e8"},{dark:"#204c61",accent:"#8ee2e7",soft:"#e5f6f7"},{dark:"#3d315b",accent:"#c5a9ff",soft:"#f0eaff"},{dark:"#5b4327",accent:"#ffd36a",soft:"#fff5d8"}];
function paletteFor(value:string){let hash=0;for(let i=0;i<value.length;i++)hash=(hash*31+value.charCodeAt(i))>>>0;return palettes[hash%palettes.length]}
function StickerArtwork({sticker,large=false}:{sticker:Sticker;large?:boolean}){const palette=paletteFor(sticker.section);return <div className={`relative overflow-hidden ${large?"aspect-[4/3] rounded-2xl":"aspect-[4/3]"}`} style={{background:`linear-gradient(145deg,${palette.soft},#fff 62%)`}}><div className="absolute -right-8 -top-10 h-28 w-28 rounded-full opacity-80" style={{background:palette.accent}}/><div className="absolute -bottom-12 -left-10 h-32 w-32 rotate-12 rounded-[2rem]" style={{background:palette.dark}}/><div className="absolute inset-x-5 top-1/2 h-px -rotate-[18deg] opacity-70" style={{background:palette.dark}}/><span className="absolute left-3 top-3 rounded-md bg-white/90 px-2 py-1 font-mono text-xs font-black shadow-sm">#{sticker.code}</span><div className="absolute inset-0 grid place-items-center"><div className={`${large?"h-24 w-24 text-3xl":"h-16 w-16 text-xl"} grid place-items-center rounded-full border bg-white/75 font-black shadow-lg`} style={{borderColor:palette.accent,color:palette.dark}}>{initials(sticker.name)}</div></div>{sticker.quantity&&<span className="absolute right-3 top-3 rounded-full bg-[#ff6a3d] px-2 py-1 text-xs font-black text-white">×{sticker.quantity}</span>}<span className="absolute bottom-3 right-3 max-w-[65%] truncate rounded-full px-2 py-1 text-[9px] font-black uppercase tracking-wider text-white" style={{background:palette.dark}}>{sticker.category||"Colección base"}</span></div>}
function initials(name:string|null){if(!name)return "CN";const words=name.split(/\s+/).filter(Boolean);return `${words[0]?.[0]||""}${words.length>1?words[words.length-1][0]:words[0]?.[1]||""}`.toUpperCase()}
