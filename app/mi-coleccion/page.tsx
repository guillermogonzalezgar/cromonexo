...
export default async function CollectionPage() {
  ...
  const [{ data: catalog, error: catalogError }, { data: userStickers, error: userError }] = await Promise.all([
    supabase.from("stickers").select("id, number, name, team, category").eq("collection_id", collection.id).order("team").order("number"),
  ...
  const stickers: Sticker[] = (catalog ?? []).map(item => {
