...
export default async function NewProposalPage({ searchParams }: { searchParams: Promise<{ usuario?: string; coleccion?: string }> }) {
  const { usuario, coleccion } = await searchParams; if (!usuario) redirect("/mis-matches");
  const supabase=await createClient(); const {data:{user}}=await supabase.auth.getUser(); if(!user) redirect("/login");
  const {data:collection}=await supabase.from("collections").select("id,name,slug").eq("slug",coleccion||"laliga-este-2026-27").single(); if(!collection) redirect("/mis-matches");
