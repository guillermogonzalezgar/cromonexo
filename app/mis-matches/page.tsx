...
export default async function MatchesPage() {
  ...
  return <div className="min-h-screen pb-24 md:pb-12">
    ...
    <main className="mx-auto max-w-6xl px-4 py-9 md:px-8 md:py-14">
      <div className="mb-8"><p className="mb-2 text-xs font-bold uppercase tracking-[.18em] text-[#527060]">{collection.name}</p><h1 className="text-4xl font-black tracking-[-.05em] md:text-5xl">Mis matches</h1><p className="mt-3 max-w-2xl text-[#607066]">Hay match cuando otra persona tiene al menos un cromo que te falta y tú tienes al menos uno de los que busca. No es necesario que coincidan todos.</p>{matches.length>0&&<p className="mt-4 inline-flex rounded-full bg-[#e5efd9] px-4 py-2 text-sm font-black text-[#164f35]">{matches.length} {matches.length===1?"match encontrado":"matches encontrados"}</p>}</div>
