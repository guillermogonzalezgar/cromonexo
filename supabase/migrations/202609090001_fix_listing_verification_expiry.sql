-- Evita la ambigüedad entre la columna expires_at y el parámetro de salida
-- del mismo nombre en start_listing_verification.
...
begin
  ...
  update public.listing_verifications verification
  set status='expired'
  where verification.seller_id=auth.uid()
  ...
  loop
    ...
    exit when not exists(
      select 1 from public.listing_verifications verification where verification.code=new_code
    );
  end loop;
